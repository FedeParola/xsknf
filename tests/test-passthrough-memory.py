...
RES_FILENAME       = 'res-passthrough-memory.csv'
RUNS               = 5
RETRIES            = 10
...
out.write("run,size,mode,throughput,target,llc-loads,llc-load-misses,llc-store,llc-store-misses,user,system,softirq,verified\n")

for run in range(RUNS):
    for size in TEST_SIZES:
        for mode in MODES:
            print(f'Run {run}: measuring size {size} in mode {mode}...')
            ...
            out.write(f'{run},{size},{mode},{best_r},{best_t},{loads},{load_misses},{stores},{store_misses},{user},{system},{softirq},{verified}\n')
            out.flush()
