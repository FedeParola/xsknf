...
ACL_SIZES          = [1, 10, 100, 1000, 10000, 100000, 1000000]
MODES              = ['xdp', 'af_xdp-bp', 'combined-bp', 'combined-poll']
FLAGS              = {
                      'xdp': ['-M', 'XDP'],
                      'af_xdp': [],
                      'af_xdp-bp': ['-B'],
...
for run in range(RUNS):
    for acl_size in ACL_SIZES:
        ...
        for mode in MODES:
            print(f'Run {run}: measuring ACL size {acl_size} in mode {mode}...')
            ...
            for retry in range(RETRIES):
                ...
                cmd = ['ssh', TESTER, 'sudo', MOONGEN_PATH, PKTGEN_SCRIPT_PATH,
                        ...
                        '-t', str(FINAL_TRIAL_TIME), '-f', str(acl_size)]
